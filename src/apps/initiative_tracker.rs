                ...
                .show(ctx, |ui| {
                    ...
                    egui::Window::new("Resolve Initiative Tie")
                        ...
                        .show(ui.ctx(), |ui| {
                            ...
                            let tied_indices: Vec<usize> = self.entries.iter()
                                .enumerate()
                                .filter_map(|(i, entry)| {
                            ...
                            for i in 0..tied_indices.len() {
                                ui.horizontal(|ui| {
                                    ...
                                    if ui.add_enabled(i < tied_indices.len() - 1, egui::Button::new("v")).clicked() {
                                        // Mark indices to be swapped after the loop
                                        swap_indices = Some((tied_indices[i], tied_indices[i+1]));
                                    }
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ui.horizontal(|ui| {
                ...
                if ui.button("Sort by Initiative").clicked() {
                    ...
                    self.active_index = if self.entries.is_empty() { None } else { Some(0) };
                ...
                for i in 0..self.entries.len() {
                    ...
                    egui::Frame::group(ui.style()).stroke(stroke).show(ui, |ui| {
                        ...
                        .show(ui, |ui| {
                            ...
                            ui.horizontal(|ui| {
                                ui.label("Init:");
                                ui.add(egui::DragValue::new(&mut self.entries[i].initiative));
                                
                                ui.separator(); // A small vertical line
                                
                                ui.label("HP:");
                                // A DragValue for the current HP
                                ui.add(egui::DragValue::new(&mut self.entries[i].hp_current));
                                ui.label("/");
                            ...
                            ui.horizontal(|ui| {
                                ui.add(egui::DragValue::new(&mut self.entries[i].hp_update));
                                ...
                                if ui.button("Update").clicked() {
                                    self.entries[i].hp_current += self.entries[i].hp_update * self.entries[i].update_sign;
                                    self.entries[i].hp_current = self.entries[i].hp_current.clamp(0, self.entries[i].hp_total);
                                }
                            });
                            ui.end_row();
